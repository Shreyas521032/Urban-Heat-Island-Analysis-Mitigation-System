...
def load_sample_data():
    ...
    np.random.seed(42)
    lat_center, lon_center = 40.7128, -74.0060  # NYC coordinates
    n_points = 500
...
def optimize_interventions(data, budget_level='medium', priority='temperature'):
    ...
    budget_map = {'low': 100, 'medium': 250, 'high': 500}
    budget = budget_map.get(budget_level, 250)
    
    # Define intervention options
    interventions = [
        {'name': 'Tree Planting', 'type': 'Green Infrastructure', 'cost_per_unit': 10, 
         'temp_reduction_per_unit': 0.05, 'max_units': 30},
        {'name': 'Cool Roofs', 'type': 'High-Albedo Surfaces', 'cost_per_unit': 15, 
         'temp_reduction_per_unit': 0.08, 'max_units': 20},
        {'name': 'Cool Pavements', 'type': 'High-Albedo Surfaces', 'cost_per_unit': 20, 
         'temp_reduction_per_unit': 0.06, 'max_units': 15},
        {'name': 'Green Roofs', 'type': 'Green Infrastructure', 'cost_per_unit': 25, 
         'temp_reduction_per_unit': 0.1, 'max_units': 12},
        {'name': 'Water Features', 'type': 'Water Features', 'cost_per_unit': 30, 
...
def show_dashboard(data):
    ...
    st.selectbox("Select City", ["New York City (Demo)"], index=0)
    ...
    with tab1:
        ...
        st.write("This heat map shows the temperature distribution across the urban area.")
